...
export const login = catchAsync(async (req, res, next) => {
  if (!req.body.password || !req.body.tinNumber) {
    return next(new AppError('Please fill empty fields!', 400));
  }

  let currentUser = await User.findOne({ where: { tinNumber: req.body.tinNumber } });
  ...
  if (!hashedPassword)
    return next(new AppError('Wrong tinNumber or password!', 401));
...
export const signup = catchAsync(async (req, res, next) => {
  ...
  let userTinExist = await User.findOne({ where: { tinNumber: req.body.tinNumber } });

  if (userTinExist) return next(new AppError('This TIN has been already registered!', 409));
  ...
  createNotification(
    createUser.id,
    'welcome to EBM Claims',
    'Your EBM Security is our concern',
  );
});
